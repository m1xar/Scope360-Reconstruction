...
- `go test ./pkg/...` — фейковый OKX-сервер (`pkg/okx/service/reconstructor/dataset_test.go`)
  проверяет, что `Load(scope.All)` дёргает каждый эндпоинт один раз и что
  результаты узких scope совпадают с полным; тест стоп-политики обхода
  Binance (`walk_test.go`).
